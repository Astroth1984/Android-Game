...
import static emobadara.ensias.com.memorygame.Assets.DailyRoutine4Assets.homework;
import static emobadara.ensias.com.memorygame.Assets.DailyRoutine4Assets.dinner;
import static emobadara.ensias.com.memorygame.Assets.DailyRoutine4Assets.bed;

public class DailyLife4 extends Screen {
    ...
    public DailyLife4(Game game) {
        super(game);
        addSprite(new Sprite(background,0,0,game.getGraphics().getHeight(),game.getGraphics().getWidth()));

        daily1= new EventContainer(homework,hx,hx/6,game.getGraphics().getHeight()-h,game.getGraphics().getWidth()-2*hx,2,true);
        addSprite(daily1);
        daily2= new EventContainer(dinner,hx,hx/6,game.getGraphics().getHeight()-h,game.getGraphics().getWidth()-2*hx,3,true);
        daily3= new EventContainer(bed,hx,hx/6,game.getGraphics().getHeight()-h,game.getGraphics().getWidth()-2*hx,4,true);
    ...
    public void handleTouchUp(int x, int y, int pointer) {
        ...
        if (btn.onTouchUp(x,y)){
            ...
            GameActivity.daily4audio1.start();

        }else
        if (btn2.onTouchUp(x,y)) {

            GameActivity.daily4audio2.start();
        }else
        if (btn3.onTouchUp(x,y)) {

            GameActivity.daily4audio3.start();
        }

        if(btn_back3.onTouchUp(x,y)) {

            GameActivity.daily4audio1.pause();
        ...
        if(btn_back.onTouchUp(x,y)) {
            GameActivity.daily4audio2.pause();
        ...
        if (btn_back2.onTouchUp(x,y)) {
            GameActivity.daily4audio3.pause();
        ...
        if (btn_next3.onTouchUp(x,y)){
            ...
            game.setScreen(new ArrangeScreen4(game));
