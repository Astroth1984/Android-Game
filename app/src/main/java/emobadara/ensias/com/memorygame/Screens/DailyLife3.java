...
import static emobadara.ensias.com.memorygame.Assets.DailyRoutine3Assets.classes;
import static emobadara.ensias.com.memorygame.Assets.DailyRoutine3Assets.lunch;
import static emobadara.ensias.com.memorygame.Assets.DailyRoutine3Assets.play;

public class DailyLife3 extends Screen {
    ...
    public DailyLife3(Game game) {
        super(game);
        addSprite(new Sprite(background,0,0,game.getGraphics().getHeight(),game.getGraphics().getWidth()));

        daily1= new EventContainer(classes,hx,hx/6,game.getGraphics().getHeight()-h,game.getGraphics().getWidth()-2*hx,2,true);
        addSprite(daily1);
        daily2= new EventContainer(lunch,hx,hx/6,game.getGraphics().getHeight()-h,game.getGraphics().getWidth()-2*hx,3,true);
        daily3= new EventContainer(play,hx,hx/6,game.getGraphics().getHeight()-h,game.getGraphics().getWidth()-2*hx,4,true);
    ...
    public void handleTouchUp(int x, int y, int pointer) {
        ...
        if (btn.onTouchUp(x,y)){
            ...
            GameActivity.daily2audio3.start();

        }else
        if (btn2.onTouchUp(x,y)) {

            GameActivity.daily3audio2.start();
        }else
        if (btn3.onTouchUp(x,y)) {

            GameActivity.daily3audio3.start();
        }

        if(btn_back3.onTouchUp(x,y)) {

            GameActivity.daily2audio3.pause();
        ...
        if(btn_back.onTouchUp(x,y)) {
            GameActivity.daily3audio2.pause();
        ...
        if (btn_back2.onTouchUp(x,y)) {
            GameActivity.daily3audio3.pause();
        ...
        if (btn_next3.onTouchUp(x,y)){
            ...
            game.setScreen(new ArrangeScreen3(game));
