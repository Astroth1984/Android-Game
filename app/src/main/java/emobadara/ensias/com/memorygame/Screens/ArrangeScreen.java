...
import static emobadara.ensias.com.memorygame.Assets.Assets.learn;
...
public class ArrangeScreen extends Screen {
    ...
    public ArrangeScreen(Game game) {
        super(game);
    ...
    private void putContainers() {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 1; j++) {
                ...
                container[i][j] = new EventContainer(Assets.cont, i * w_unitc, j * h_unitc, h_unitc, w_unitc, i + 1, true);
        ...
        e1 = new Event(DailyRoutineAssets.breakfast, w_unitc / 3, 1 * h_unitc, h_unitc, wu, 2);
        addSprite(e1);
        events.add(e1);
        e2 = new Event(DailyRoutineAssets.wakeup, w_unitc / 3 + wu, 1 * h_unitc, h_unitc, wu, 1);
        addSprite(e2);
        events.add(e2);
        e3 = new Event(DailyRoutineAssets.test, w_unitc / 3 + 2 * wu, 1 * h_unitc, h_unitc, wu, 3);
        addSprite(e3);
        events.add(e3);
    ...
    public void handleTouchUp(int x, int y, int pointer) {
        ...
        if (learn.onTouchUp(x, y)) {
            game.setScreen(new DailyLife(game));

        } else if (button_back.onTouchUp(x, y)) {
        ...
        if (isInserted(containers, events)  ) {
            if (isCorrect(containers,events)) {
                Intent intent = new Intent(GameActivity.activity, DiffThemes.class);
                GameActivity.activity.startActivity(intent);
                GameActivity.activity.finish();
                DiffThemes.diffthemes.finish();
        ...
        } //else game.setScreen(new DailyLife(game));
    }
    ...
    boolean isCorrect(List<EventContainer> list, List<Event> events) {
        boolean correct = false;
        for (EventContainer ec : list) {
            for (Event event : events) {
                if (ec.screennum==event.type) {
                    correct=true;
                }
            }
        }
