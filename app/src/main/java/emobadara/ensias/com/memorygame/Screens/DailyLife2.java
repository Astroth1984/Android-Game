...
import static emobadara.ensias.com.memorygame.Assets.DailyRoutine2Assets.classes;
import static emobadara.ensias.com.memorygame.Assets.DailyRoutine2Assets.gotoschool;
import static emobadara.ensias.com.memorygame.Assets.DailyRoutine2Assets.dressed;

public class DailyLife2 extends Screen {
    ...
    public DailyLife2(Game game) {
        super(game);
        addSprite(new Sprite(background,0,0,game.getGraphics().getHeight(),game.getGraphics().getWidth()));

        daily1= new EventContainer(dressed,hx,hx/6,game.getGraphics().getHeight()-h,game.getGraphics().getWidth()-2*hx,2,true);
        addSprite(daily1);
        daily2= new EventContainer(gotoschool,hx,hx/6,game.getGraphics().getHeight()-h,game.getGraphics().getWidth()-2*hx,3,true);
        daily3= new EventContainer(classes,hx,hx/6,game.getGraphics().getHeight()-h,game.getGraphics().getWidth()-2*hx,4,true);
    ...
    public void handleTouchUp(int x, int y, int pointer) {
        ...
        if (btn.onTouchUp(x,y)){
            ...
            GameActivity.daily2audio1.start();

        }else
        if (btn2.onTouchUp(x,y)) {

            GameActivity.daily2audio2.start();
        }else
        ...
        if(btn_back3.onTouchUp(x,y)) {
            ...
            GameActivity.daily2audio1.pause();
        ...
        if(btn_back.onTouchUp(x,y)) {
            GameActivity.daily2audio2.pause();
